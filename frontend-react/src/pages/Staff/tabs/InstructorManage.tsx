// src/pages/Staff/tabs/InstructorManagerTab.tsx
...
function Avatar({
  src, name, size = 40, fontSize = "1rem",
}: { src: string | null; name: string; size?: number; fontSize?: string }) {
  ...
  return (
    <div style={{
      width: size, height: size, borderRadius: "50%", background: "linear-gradient(135deg, #4f46e5, #7c3aed)",
...
export default function InstructorManagerTab() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      if (instrRes.status === "fulfilled") {
        ...
        // Fetch photos as blobs for instructors that have profile_url
        sorted.forEach(i => {
          if (i.profile_url) {
  ...
  // Build department options from departments table
  const deptOptions = [
    ...new Set([
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem", color: "black" }}>
      ...
      {/* Header */}
      <div>
        <h2 style={{ fontSize: "1rem", fontWeight: 600, color: "#1f2937" }}>Instructor Manager</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: "1.25rem", alignItems: "start" }}>
        ...
        <div style={{ ...card, display: "flex", flexDirection: "column" }}>
          <div style={{ padding: "0.875rem", borderBottom: "1px solid #f3f4f6", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            <input
              type="text" placeholder="🔍 Search name, ID..." value={search}
              onChange={e => setSearch(e.target.value)}
              style={{ padding: "0.5rem 0.75rem", border: "1px solid #e5e7eb", borderRadius: "0.5rem", fontSize: "0.8rem", outline: "none", width: "100%", boxSizing: "border-box" }}
            />
          ...
          <div style={{ maxHeight: "560px", overflowY: "auto" }}>
            {loadingList ? (
              ...
              <div style={{ padding: "2rem", textAlign: "center", color: "#9ca3af", fontSize: "0.8rem" }}>No instructors found</div>
            ) : filtered.map(i => {
              ...
              return (
                <button key={i.id} onClick={() => setSelected(i)}
                  style={{
                    ...
                    borderLeft: `3px solid ${isSelected ? "#4f46e5" : "transparent"}`,
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: "0.8rem", fontWeight: 600, color: "#1f2937", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {i.name}
                    </p>
                    <p style={{ fontSize: "0.68rem", color: "#a5b4fc", fontFamily: "monospace" }}>{i.instructor_id}</p>
                    <p style={{ fontSize: "0.68rem", color: "#9ca3af", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          ...
          <div style={{ padding: "0.5rem 0.875rem", borderTop: "1px solid #f3f4f6", background: "#f9fafb", fontSize: "0.7rem", color: "#9ca3af" }}>
        ...
        {!selected ? (
          <div style={{ ...card, padding: "4rem 2rem", textAlign: "center" }}>
            ...
            <p style={{ fontWeight: 600, color: "#1f2937" }}>Select an instructor</p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            <div style={card}>
              <div style={{ background: "linear-gradient(135deg, #4f46e5, #7c3aed)", padding: "1.25rem 1.5rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "0.75rem" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                    ...
                    <div>
                      ...
                      <p style={{ color: "#c4b5fd", fontSize: "0.75rem", fontFamily: "monospace", marginTop: "0.2rem" }}>{selected.instructor_id}</p>
                      <p style={{ color: "#a5b4fc", fontSize: "0.75rem", marginTop: "0.2rem" }}>
                        {selected.department || "No department"}
                        {selected.specialization ? ` · ${selected.specialization}` : ""}
                      </p>
                      <p style={{ color: "#7c6fcd", fontSize: "0.7rem", marginTop: "0.15rem" }}>{selected.email}</p>
                    </div>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: "0.625rem", alignItems: "center", flexWrap: "wrap" }}>
                    ...
                    <button onClick={handleExport} disabled={logs.length === 0}
                      style={{
                        ...
                        padding: "0.5rem 1rem",
                        background: logs.length === 0 ? "rgba(255,255,255,0.1)" : "#fff",
                        color: logs.length === 0 ? "rgba(255,255,255,0.4)" : "#4f46e5",
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)" }}>
                {[
                  ...
                  <div key={s.label} style={{ padding: "0.875rem", textAlign: "center", borderRight: "1px solid #f3f4f6" }}>
            ...
            <div style={card}>
              <div style={{ padding: "0.875rem 1.25rem", borderBottom: "1px solid #f3f4f6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <p style={{ fontSize: "0.8rem", fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                  {new Date(monthFilter + "-01").toLocaleDateString("en-PH", { month: "long", year: "numeric" })}
                </p>
                <span style={{ fontSize: "0.75rem", background: "#eef2ff", color: "#4f46e5", padding: "0.2rem 0.625rem", borderRadius: "9999px", fontWeight: 600 }}>
                    ...
                    <thead>
                      <tr style={{ background: "#f9fafb" }}>
                        {["#", "Date", "Day", "Subject", "Code", "Room", "Time In", "Time Out", "Status"].map(h => (
                          <th key={h} style={{ padding: "0.75rem 1rem", textAlign: "left", fontSize: "0.7rem", fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em", whiteSpace: "nowrap" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {logs.map((log, idx) => (
                        <tr key={log.id} style={{ borderTop: "1px solid #f3f4f6" }}
                          ...
                          <td style={{ padding: "0.75rem 1rem", fontWeight: 500, whiteSpace: "nowrap" }}>{log.date}</td>
                          ...
                          <td style={{ padding: "0.75rem 1rem", maxWidth: "200px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{log.subject || "—"}</td>
                          <td style={{ padding: "0.75rem 1rem", fontFamily: "monospace", fontSize: "0.78rem", color: "#6b7280" }}>{log.code || "—"}</td>
                          <td style={{ padding: "0.75rem 1rem", color: "#6b7280" }}>{log.room || "—"}</td>
                          <td style={{ padding: "0.75rem 1rem", color: "#6b7280", whiteSpace: "nowrap" }}>{log.time_in || "—"}</td>
                          <td style={{ padding: "0.75rem 1rem", color: "#6b7280", whiteSpace: "nowrap" }}>{log.time_out || "—"}</td>
                          <td style={{ padding: "0.75rem 1rem" }}>
