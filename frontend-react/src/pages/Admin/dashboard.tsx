...
const addDropdownItems = [
  {
    ...
    icon: (<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="8.5" cy="7" r="4"/><line x1="20" y1="8" x2="20" y2="14"/><line x1="23" y1="11" x2="17" y2="11"/></svg>),
  },
  {
    key: "add-staff", label: "Add Staff",
    icon: (<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/><line x1="20" y1="8" x2="20" y2="14"/><line x1="23" y1="11" x2="17" y2="11"/></svg>),
  },
  {
...
const settingsDropdownItems = [
  ...
  },
  {
    ...
    icon: (<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 3H5a2 2 0 00-2 2v4m6-6h10a2 2 0 012 2v4M9 3v18m0 0h10a2 2 0 002-2v-4M9 21H5a2 2 0 01-2-2v-4m0 0h18"/></svg>),
  },
  {
...
export default function AdminDashboard() {
  ...
  const navBtn: React.CSSProperties = {
    background: "#312e81", border: "none", color: "#fff",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f9fafb", fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      ...
      {showLogout && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
          <div style={{ background: "#fff", borderRadius: "1rem", padding: "2rem", maxWidth: "24rem", width: "100%", boxShadow: "0 25px 50px rgba(0,0,0,0.25)", textAlign: "center" }}>
            ...
            <p style={{ fontSize: "0.875rem", color: "#6b7280", marginBottom: "1.5rem" }}>Are you sure you want to logout?</p>
      ...
      <nav style={{ background: "#3730a3", color: "#fff", boxShadow: "0 4px 6px -1px rgba(0,0,0,0.1)" }}>
        <div style={{ maxWidth: "80rem", margin: "0 auto", padding: "0 1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center", height: "4rem" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
            ...
            <div>
              <p style={{ fontWeight: 700, fontSize: "0.95rem", lineHeight: 1.2 }}>Admin Dashboard</p>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.625rem" }}>
            ...
            <div ref={addRef} style={{ position: "relative" }}>
              <button onClick={() => { setAddOpen(p => !p); setSettingsOpen(false); }} title="Add"
                style={{ ...navBtn, background: ADD_KEYS.includes(activeTab) || addOpen ? "#4338ca" : "#312e81" }}
                onMouseEnter={e => (e.currentTarget.style.background = "#4338ca")}
                onMouseLeave={e => { if (!ADD_KEYS.includes(activeTab) && !addOpen) e.currentTarget.style.background = "#312e81"; }}>
              ...
              {addOpen && (
                <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, background: "#fff", border: "1px solid #e5e7eb", borderRadius: "0.5rem", boxShadow: "0 10px 15px -3px rgba(0,0,0,0.15)", minWidth: "180px", zIndex: 50, overflow: "hidden" }}>
                  {addDropdownItems.map((item, idx) => (
                    <button key={item.key} onClick={() => { setActiveTab(item.key); setAddOpen(false); }}
                      style={{ width: "100%", display: "flex", alignItems: "center", gap: "0.6rem", padding: "0.625rem 1rem", border: "none", borderBottom: idx < addDropdownItems.length - 1 ? "1px solid #f3f4f6" : "none", background: activeTab === item.key ? "#eef2ff" : "#fff", color: activeTab === item.key ? "#4f46e5" : "#374151", fontSize: "0.875rem", fontWeight: activeTab === item.key ? 600 : 400, cursor: "pointer", textAlign: "left" }}
            ...
            <div ref={settingsRef} style={{ position: "relative" }}>
              <button onClick={() => { setSettingsOpen(p => !p); setAddOpen(false); }} title="Settings"
                style={{ ...navBtn, background: SETTINGS_KEYS.includes(activeTab) || settingsOpen ? "#4338ca" : "#312e81" }}
                onMouseEnter={e => (e.currentTarget.style.background = "#4338ca")}
      ...
      <div style={{ maxWidth: "80rem", margin: "0 auto", padding: "1.5rem" }}>
        ...
        <div style={{ display: activeTab === "instructors"    ? "block" : "none" }}><InstructorsTab setActiveTab={setActiveTab} /></div>
        <div style={{ display: activeTab === "staff"          ? "block" : "none" }}><StaffTab setActiveTab={setActiveTab} /></div>
        <div style={{ display: activeTab === "events"         ? "block" : "none" }}><EventsTab /></div>
        <div style={{ display: activeTab === "schedules"      ? "block" : "none" }}><SchedulesTab /></div>
        <div style={{ display: activeTab === "add-instructor" ? "block" : "none" }}><AddInstructorTab /></div>
        <div style={{ display: activeTab === "add-staff"      ? "block" : "none" }}><AddStaffTab /></div>
        <div style={{ display: activeTab === "add-subject"    ? "block" : "none" }}><AddSubjectTab /></div>
        <div style={{ display: activeTab === "add-department" ? "block" : "none" }}><AddDepartmentTab /></div>
        <div style={{ display: activeTab === "security"       ? "block" : "none" }}><SecurityTab /></div>
        <div style={{ display: activeTab === "device"         ? "block" : "none" }}><DeviceTab /></div>
